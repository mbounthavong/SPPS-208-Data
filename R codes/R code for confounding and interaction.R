################################################################################
# Title:        R tutorial - Confounding and Interaction example 
# By:           Mark Bounthavong
# Output:       R 
...
#### This tutorial will provide an introduction to performing Linear Regression (e.g., OLS) models and adjusting for confounding and investigating interaction effects. 
...
### Interaction effects
#### Generate groups based on insulin (no-insulin = 0, insulin = not 0)
diabetes.data$insulin.group[diabetes.data$Insulin == 0] = 0
diabetes.data$insulin.group[diabetes.data$Insulin != 0] = 1
diabetes.data$insulin.group

#### Descriptive interaction term (look at this as a contingency table)
CrossTable(diabetes.data$insulin.group, diabetes.data$group)

#### Multivariable linear regression model with interaction term
#### Generate the factor variables
diabetes.data$group.factor <- as.factor(diabetes.data$group)
diabetes.data$factor.insulin <- as.factor(diabetes.data$insulin.group)

#### Run the regression model
linear.model5 <- lm(Glucose ~ Age + group.factor + factor.insulin + group.factor:factor.insulin, data = diabetes.data)
summary(linear.model5)
confint(linear.model5)  ## Generates the 95% CI

#### Estimate the average marginal effects
margins1 <- margins(linear.model5, type = "response", data = diabetes.data, at = (list(group.factor = c("0", "1"), factor.insulin = c("0", "1"))))
summary(margins1)


### Plot two interaction terms against separate against age (note the difference in the slopes are not parallel)
plot.interact1 <- qplot(x = Age, y = Glucose, color = group.factor, data = diabetes.data) + stat_smooth(method = "lm", se = FALSE, fullrange = TRUE)
plot.interact1

plot.interact2 <- qplot(x = Age, y = Glucose, color = factor.insulin, data = diabetes.data) + stat_smooth(method = "lm", se = FALSE, fullrange = TRUE)
plot.interact2
